...
exports.default = {
    ...
    watch: {
        codeMode: function codeMode(val) {
            this.setMode();
        }
    }
}
// </script>
;
exports.default.template = '<div id="codeBox" class="code-box"><div class="title-box input-group-sm"><input placeholder="请输入标题" v-model="codeTiTle" class="form-control codebox-title"/><select v-model="codeMode" class="form-control codebox-mode"><option v-for="m in modeList" value="{{m}}">{{m}}</option></select><div class="btn-group"><label @click="$root.expandBox = !$root.expandBox" class="btn btn-default codebox-save"><i :class="{\'fa-expand\': !$root.expandBox, \'fa-compress\': $root.expandBox}" class="fa fa-expand"></i></label></div><div class="btn-group"><label @click="save" class="btn btn-default codebox-save"><i class="iconfont">&#xe603;</i></label></div><div style="margin-right: 5px;" class="btn-group"><label @click="del" class="btn btn-default codebox-save"><i class="iconfont">&#xe605;</i></label></div></div><tag-select :tag-list="tagList" :tag-sels="tagSels"></tag-select><div class="codemirror-box input-group-sm"><pre id="aceEditor"></pre></div></div>';