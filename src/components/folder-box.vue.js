...
exports.default = {
    ...
    methods: {
        openContextMenu: function openContextMenu(id) {
            var _this = this;

            this.contextMenuId = id;
            this.menu.items[4].visible = this.treeMap[id].parent != 'root';
            setTimeout(function () {
                _this.menu.popup(remote.getCurrentWindow());
            }, 100);
        },
    ...
    ready: function ready() {
        ...
        $doc.on('click', function (e) {
            ...
            if ($elm.find(e.target).length == 0) {
                _this2.contextMenuId = _this2.editItemId = -1;
            } else {
        ...
        $elm.on('click', '.tree-content', function () {
            ...
            if (classList.contains('tree-drag-in')) {
                ...
                self.treeMap[model.parent].children.$remove(model);
                var item = self.treeMap[this.dataset.id];
                var index = self.treeMap[item.parent].children.findIndex(function (n) {
                    return n.id == item.id;
                });
        ...
        this.menu = Menu.buildFromTemplate([{
            ...
            label: '删除',
            click: function click() {
                message.open([{ name: '确定', action: function action() {
    ...
    watch: {
        editItemId: function editItemId(val) {
            var _this3 = this;
            ...
            if (val != -1) {
                ...
                Vue.nextTick(function () {
                    var ipt = $(_this3.$el).find('[data-id="' + val + '"] input')[0];
                    ipt.select();
                });
            } else {
...
// </script>
;
exports.default.template = '<div id="ideFolderTree" class="tree-box"><ul @keyup.enter="editItem(editItemId)" class="tree-root"><item :model="treeData" :deep="deep" class="item"></item></ul></div>';